...
drawEditLIGUI() {
    ...
    Loop % (LINumHotkeys + NumHotkeys) {
        ...
        if (A_Index < (LINumHotkeys + LINumHotkeysAdd + NumHotkeys) && A_Index > ((LINumHotkeys + LINumHotkeysAdd + NumHotkeys) - 3)) {
            ...
            if (A_Index > LINumHotkeys && A_Index <= (LINumHotkeys + LINumHotkeysAdd)) {
                ;Gui, EditGui: Add, Checkbox, vHKEnable v1HKEnable%A_Index% xp+70, Enable?
            }
        }
        ypos += 25
    }
...
drawEditCuboidsGUI() {
    ...
    Loop % (CuboidsNumHotkeys + NumHotkeys){
        ...
        Gui, EditGui: Font, s8, Segoe UI
        Gui, EditGui: Add, Edit, Disabled vCuboidsHotkeyName1%A_Index% w110 xp+155 y%ypos%, None
        Gui, EditGui: Add, Edit, Disabled vCuboidsHotkeyName2%A_Index% w110 xp+130 yp-1, None
        Gui, EditGui: Font,, 

        if (A_Index < (CuboidsNumHotkeys + NumHotkeys) && A_Index > ((CuboidsNumHotkeys + NumHotkeys) - 3)) {
            Gui, EditGui: Font, s10 bold, Segoe UI
            Gui, EditGui: Add, text, cBlack xp+120, Not customizable
            Gui, EditGui: Font,,
        } else {
            Gui, EditGui: Add, Button, gBind1 v5Bind1%A_Index% yp-1 xp+110, Set Primary 
            Gui, EditGui: Add, Button, gBind2 v5Bind2%A_Index% xp+70, Set Secondary 
            Gui, EditGui: Add, Button, gReset v5Reset%A_Index% xp+85, Reset Key
        }
        ypos += 25
    }
    return
